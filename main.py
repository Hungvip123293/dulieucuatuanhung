...
#Gorgon=exec(requests.get('https://raw.githubusercontent.com/Hungvip123293/dulieucuatuanhung/main/xgorgon.py').text)
        ...
        try:
            ...
            try: 
             if response.json()['status_code'] == 0:
                self.__success += 1
                self.__safe_print(f" ~ Views sent: "+Col.purple+"["+Col.reset+f"{self.__success}"+Col.purple+"]"+Col.blue+" </>"+Col.reset+f" {str(response.json()['log_pb']['impr_id']).lower()}")
             else:
                self.__errors += 1
            except:
             pass
